<!--
 * @Author: Hexg
 * @Date: 2019-07-10 11:48:05
 * @LastAuthor: 陈艳玲
 * @lastTime: 2019-11-25 15:13:54
 -->

<template>
  <div class="visiting-card">
    <Form :label-width="120" :model="data" :rules="ruleValidate" ref="form">
      ...
      <FormItem label="LOGO：">
        <Tooltip placement="right" content="点击修改">
          <div class="img-box daq-no-photo">
            <img
              v-if="data.logo && data.logo.indexOf('http') !== -1"
              :src="data.logo"
              class="daq-img"
              alt
            />
...
import {
  getSiteVisitingCard,
  saveSiteVisitingCard,
  SiteVisitingCardDataType
...
export default class VisitingCard extends Vue {
  ...
  ruleValidate = {
    name: [{ required: true, message: "名片名称不能为空", trigger: "blur" }],
    images: [{ required: true, message: "请上传名片主图" }]
  };
  ...
  private saveCard() {
    (this.$refs.form as any).validate((valid: any) => {
      ...
      saveSiteVisitingCard(this.data)
        .then(res => {
          if (res.code === 0) {
  ...
  // 获取详情
  getDetail() {
    getSiteVisitingCard().then(res => {
      if (res.code === 0) {
        this.data = res.data;
        if (res.data.video) {
          let item: Array<VedioListType> = [
            { image: res.data.videoCover, url: res.data.video }
          ];
          this.videoData = item;
        }
        // this.videoData.image = res.data.videoCover as string
        // this.videoData.url = res.data.video as string
      }
    });
  }
  /**
