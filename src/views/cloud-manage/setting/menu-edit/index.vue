  ...
  <div class="people">
    <Form :label-width="90" :model="form" :rules="validate" ref="form">
      <FormItem label="菜单名称：" prop="name">
        <Input v-model="form.name" class="ipt-max" :maxlength="10" placeholder="请输入菜单名称"/>
      </FormItem>
      <FormItem label="菜单编码：">
      ...
      <FormItem label="上级菜单：">
        <Select v-model="form.pid" filterable class="ipt-max">
          ...
          <Option v-for="menu in menuList" :key="menu.id" :value="menu.id" :disabled="menu.id === form.id">{{menu.name}}</Option>
        </Select>
      </FormItem>
      <FormItem label="菜单地址：">
      ...
      <FormItem label="排序：" prop="sort">
        <Input class="ipt-max" number type="number" :maxlength="10" v-model="form.sort" placeholder="请输入"/>
      </FormItem>
      <FormItem>
...
import {saveMenu, checkMenuCode, getMenuList, getMenuView} from '@service/app'
...
export default class MenuAdd extends Vue {
  ...
  private validateCode (rule: any, value: any, callback: any) {
    ...
    if (value.lengt > 20) {
      return callback(new Error('菜单编码不能大于20个字符'));
    }
