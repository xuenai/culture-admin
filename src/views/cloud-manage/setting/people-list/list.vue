    ...
    <div class="article">
      <div class="top">
        <Form ref="searchForm" :model="requestParams" inline class="searchForm">
          <FormItem>
            <Input v-model.trim="requestParams.name" :maxlength="20" placeholder="账号姓名" class="tags-ipt">
...
export default class PeopleManage extends Vue {
  ...
  getList(page?: any) {
    ...
    getUserList(this.requestParams)
      .then(res => {
        ...
        if (datas) {
          this.datas = datas;
          (this.configure as any).page=res.page ? res.page : {};
        }
      })
      .catch(err => {
        this.loading = false;
  ...
  mounted() {
    ...
    this.bus.$on("selectSiteOrgId", (querySiteOrgId: string) => {
      this.requestParams.querySiteOrgId = querySiteOrgId || ''
      this.searchHandle();
    });
  }
}
