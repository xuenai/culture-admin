  ...
  <div class="tags-page">
    <div class="top-con">
      <Form inline v-model="requestParams">
        <FormItem label="标签名称：" :label-width="80">
          <Input placeholder="请输入标签名称" :maxlength="20" v-model.trim="requestParams.labelName" class="tags-ipt" />
        </FormItem>
        
        <FormItem label="标签类型：" :label-width="80" v-if="tagTypes.length">
        ...
        <Table :columns="columns" :data="datas" border :loading="loading" @on-selection-change="onTableSelectHandle">
          ...
          <template slot-scope="{ row }" slot="status">
            <div class="table-status">
              {{['正常','禁用','草稿','站点标签'][row.status - 1]}}
              <span class="switch-box" v-if="[1,2].includes(row.status)">
          ...
          <template slot-scope="{ row }" slot="operate">
            <div class="table-operate">
              ...
              <span class="tab-btn" @click="deleteNoticeHandle(row.id)">删除</span>{{ row.status === 4 ? '|' : '' }}
              <span class="tab-btn" v-if="row.status === 4" @click="toCloud(row.id)">升级为云端标签</span>
            </div>
          </template>
        </Table>
      </BaseTable>
    ...
    <Modal v-model="addTagModle" :title="modalTitle" :width="400" footer-hide>
      <Form v-model="modalData">
        ...
        <FormItem label="标签类型：" :label-width="90" v-if="tagTypes.length">
...
import { getTags, saveTags, deleteTag, updateTagsStatus, sortTags} from "@service/common";
...
import {upgradeLabel} from "@service/cloud-manage";
...
export default class CloudTags extends Vue {
  ...
  private getTagList(page?: any) {
    ...
    getActivityClassList(this.requestParams)
      .then(res => {
        this.loading = false;
        // 重置选中的数组
        this.selectedDatas = []
  ...
  private saveTag () {
    ...
    saveTags(this.modalData).then(res => {
