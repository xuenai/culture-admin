  ...
  <div class="people">
    <Form :label-width="90" :model="form" :rules="validate" ref="form">
      ...
      <FormItem label="所属机构：" prop="orgId">
        <Select class="ipt-max" v-model="form.orgId"  disabled>
      ...
      <FormItem label="场所核销：">
        <div class="ivu-transfer-list">
          <div class="ivu-transfer-list-header" style="height:36px;line-height:20px;">
            <span class="ivu-transfer-list-header-title">已授权场所</span>
            <span class="ivu-transfer-list-header-count">{{validPower.length}}</span>
          </div>
          <div class="ivu-transfer-list-body">
            <ul class="ivu-transfer-list-content">
              <li class="ivu-transfer-list-content-item"
                v-for="item in validPower"
                :key="item.resourceId"
              >
                <span class="name">{{item.name}}<span v-if="item.pname">（{{item.pname}}）</span></span>
              </li>
              <li class="ivu-transfer-list-content-not-found">暂无已授权场所</li>
            </ul>
          </div>
        </div>
      </FormItem>
  ...
  created() {
    ...
    axios.all([
      ...
      if (infoRes.code === 0 && listRes.code === 0 && orgRes.code === 0) {
        ...
        //   this.validPower.push(item);
        //   item.child.map((child: any) => {
        //     this.validPower.push(child)
        //   })
        // })
        infoRes.data.validPower.map((item:any) => {
