  ...
  <div class="container">
    ...
    <div class="topic">
      <Form class="form" ref="formInline" inline :label-width="70">
        <FormItem label="关键字">
          <Input type="text" placeholder="订单号/活动名称" v-model="params.keyword" style="width: 200px" />
        </FormItem>
        <FormItem label="预约人">
          <Input type="text" placeholder="姓名/手机" v-model="params.name" style="width: 200px" />
        </FormItem>
        <FormItem label="下单日期">
          <DatePicker
            format="yyyy-MM-dd"
        ...
        <FormItem label="预约类型">
          <Select class="w150" v-model="params.reservationType">
          <Option value="" selected>全部</Option>
          <Option value="PERSON">个人预约</Option>
    ...
    <div class="table">
      ...
      <ul class="orderHeader mt15">
        <li>
          ...
          <p class="middle">预约类型</p>
          <p class="middle">预约人</p>
          <p class="middle">到场人数</p>
            ...
            <div class="colBox">
              ...
              <div class="detail-text-info">
                ...
                <span class="detail-text-text">
                  <router-link
                    :to="{path:'/venue-order-detail',query:{id:item.id}}"
              ...
              <div class="detail-text-info">
                ...
                <span class="detail-text-text">{{item.orderIndateStart}} 至 {{item.orderIndateEnd}}</span>
              </div>
            ...
            <div class="info">
              <div class="big">
                <div class="block ml20">
                  <img :src="item.images.split(',')[0]" class="img fl mr20 daq-no-photo" />
                  <span class="credit-icon" v-if="item.faithAuditStatus==1">诚信免审</span>
                </div>
                <p class="line">
                  <span class="txt title">{{item.name}}</span>
                  <span class="txt address">最大接待人数：{{item.maxNum}}</span>
                </p>
                <p class="remark" v-if="item.remark" :title="item.remark">预约备注：{{item.remark}}</p>
              </div>
              <div class="middle">{{item.reservationType=='PERSON'?'个人预约':'团队预约'}}</div>
              <div class="middle">{{item.userName}}</div>
              <div class="middle">{{item.useNum}}</div>
              <div class="middle">
                <p>{{item.orderTimeDay}}</p>
                <p>{{item.orderTimeHourStart}}-{{item.orderTimeHourEnd}}</p>
              </div>
              <div class="middle">{{item.userPhone}}</div>
              <div class="middle">{{item.payMoney==0&&item.payIntegral==0?'免费':(item.payMoney>0?item.payMoney:item.payIntegral+'积分')}}</div>
...
import { venueOrderList, restartCode } from "@service/place";
...
export default class ComponentName extends Vue {
  ...
  private outExcelApi: string =
    CONFIG.defaultUrl + "/act/activity/order/excelVenueOrder?";
  outExcel(e: any) {
  ...
  private params: object = {
    keyword:"",//订单号/活动名称
    name:"",//预定人名称/手机号
    createTimeStart:"",//下单日期开始
    createTimeEnd:"",//下单日期结束
    reservationType:"",//预定类型
    venueOrderTimeStart:"",//场馆预定时间段开始
    venueOrderTimeEnd:"",//场馆预定时间段结束
    orderTimeStart:"",//预定日期开始
    orderTimeEnd:"",//预定日期结束
    currPage: 1, // 当前第几页
    pageSize: 10, // 当前页数量
    total: 6 // 总页数
  ...
  getList() {
    let _this = this;
    venueOrderList(_this.params).then((res: any) => {
      if (res.code === 0) {
  ...
  getOrderStartTime(date: any) {
    (this.params as any).createTimeStart = date[0] || "";
    (this.params as any).createTimeEnd = date[1] || "";
  }

  // 预约日期
  getIndateStartTime(date: any) {
    (this.params as any).orderTimeStart = date[0] || "";
    (this.params as any).orderTimeEnd = date[1] || "";
  }
  //预约时段
  getOrderTime(time:any){
    (this.params as any).venueOrderTimeStart=time[0]||"";
    (this.params as any).venueOrderTimeEnd=time[1]||"";
  }
  // tab切换
  setStatus(index: any) {
    let _this = this;
