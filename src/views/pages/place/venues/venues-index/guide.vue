  ...
  <div class="venue-guide">
    ...
    <ul class="header-goods">
      ...
      <li>
        ...
        <!--<span class="demo1 noBorder"></span>--><span>2.添加详情</span>
      </li>
    </ul>
    <!-- 选择文化场馆类型 -->
    <div class="add-content-detail-title mt20">
      <p class="title-tool">
        <i class="cloud-icon">&#xe697;</i> 选择文化场馆类型
      </p>
      <div class="add-content-detail-content">
        <Row class="ActivityType">
          <Col
            class="pd10"
            span="5"
          ...
          >
            <div
              ...
              @click.stop="chooseActivityType(item)"
            >
              <span v-if="method === item.value"
                ><i class="cloud-icon">&#xe628;</i></span
              >
              <img class="cloud-img" :src="item.image" />
              <div class="mt10">
                <p class="saleTitles ">{{ item.name }}</p>
                <p class="summary mt10" :title="item.summary">{{ item.summary }}</p>
              </div>
            </div>
          </Col>
        </Row>
      </div>
      <router-link :to="{path:'/venues-search',query:{dictType:method,name:name}}">
...
import { getVenueType } from '@service/place'
@Component
export default class BookingActivity extends Vue {
  ...
  // 选择活动方式
  chooseActivityType(item: any) {
    this.method = item.value
    this.name=item.name
  }
  //获取场馆类型
  private venueTypes: any = []
  getVenueType() {
    getVenueType({
      type: 'VENUE_TYPE'
    }).then((res: any) => {
      this.venueTypes = res.datas
    })
  }
}
</script>
