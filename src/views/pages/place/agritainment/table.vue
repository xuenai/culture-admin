      ...
      >
          ...
          <div class="clearfix food-msg">
            <div class="msg-panel">
              <p class="food-name">
                ...
                <router-link
                  :to="{path:'/agritainment-detail',query:{id:row.id,type:'list'}}"
        ...
        <template slot-scope="{row,index}" slot="action">
          <dropCode
            ...
            :id="'qrcode' + row.id"
            :key="'qrcode' + row.id"
            :value="$store.state.app.userInfo.h5Domain+'/#/homestay-detail/' +row.id"
          >
          ...
          >提交</a>
          <router-link
            :to="{path:'/agritainment-edit',query:{id:row.id,type:'list'}}"
...
import {
  ...
  deleteCol,
  submitBatch,
  agritainmentWithdraw,
  getAgritainmentStatus,
  agritainmentImportExcel
} from "@service/place";
...
export default class foodTable extends Vue {
  ...
  private outExcelApi: string =
    CONFIG.defaultUrl + "/res/agritainment/exportExcel?";
  outExcel(e: any) {
  ...
  private acceptExcel(e: any) {
    ...
    this.spinShow = true;
    agritainmentImportExcel(formData).then((blob: any) => {
  ...
  getList(data: any = this.tableConfig.page) {
    ...
    getAgritainmentList(_this.params).then((res: any) => {
    ...
    let config: any = {
      ...
      onOk: () => {
        deleteCol({
          ...
          type: "CONTENT_TYPE_AGRITAINMENT" //资源类型
        }).then((res: any) => {
          if (res.code === 0) {
  ...
  submitCol(item: any, index: any) {
    let _this = this,
      checkedLen = _this.checkedIds ? _this.checkedIds.split(",").length : 0;
    if (index === -1 && !checkedLen) {
      _this.$Message.error("请选择你要提交的农家乐");
      return;
    }
    let config: any = {
      title: "操作提示",
  ...
  mounted() {
    let _this = this;
    //搜索条件
    _this.bus.$on("requestAgritainmentList", (res: any) => {
      this.tableConfig = {
        page: {
          currPage: 1,
          pageSize: 10,
