  ...
  <div class="detail-wrap">
    ...
    <Tabs active-key="key2" class="mt20 tab-wrap" :animated="false">
      ...
      <Tab-pane label="基本资料" key="key1">
        <Row class="item mt20">
          <Col span="4" class="label-name">农家乐名称</Col>
        ...
        <Row class="item mt20">
          ...
          <Col span="20" class="item-content">{{desc.businessTime}}</Col>
        </Row>
        <Row class="item mt20">
          <Col span="4" class="label-name">服务电话</Col>
          <Col span="20" class="item-content">{{desc.servicePhone}}</Col>
        </Row>
        <Row class="item mt20">
          <Col span="4" class="label-name">包厢数</Col>
          <Col span="20" class="item-content">{{desc.boxNum}}</Col>
        </Row>
        <Row class="item mt20">
          <Col span="4" class="label-name">人均消费</Col>
          <Col span="20" class="item-content">{{desc.consumeAvg}}</Col>
        </Row>
        <Row class="item mt20">
        ...
        <!-- <Row class="item mt20">
          <Col span="4" class="label-name">诚信分值</Col>
          <Col span="20" class="item-content">{{desc.resourceCode}}</Col>
        </Row> -->
        <!--个性标签-->
        ...
        <Row class="item mt20">
          <Col span="4" class="label-name">农家乐星级</Col>
          <Col span="20" class="btn-list">
            <span
              class="choice-tag" :class="{'active': desc.level == item.id}" v-if="desc.level == item.id"
        ...
        <Row class="item mt20">
          <Col span="4" class="label-name">农家乐类型</Col>
          <Col span="20" class="btn-list">
            <span
              class="choice-tag active"
              v-for="(item,index) in tagList.type"
              :key="index"
            >{{item.labelName}}</span>
          </Col>
        </Row>
        <Row class="item mt20" v-if="desc.longitude && desc.latitude">
          <Col span="4" class="label-name">农家乐位置</Col>
      ...
      <Tab-pane label="农家乐介绍" key="key2" class="item">
        <Row class="item mt20" v-if="desc.images">
          <Col span="4" class="label-name">农家乐图片</Col>
        ...
        <Row class="item mt20">
          <Col span="4" class="label-name">农家乐概况</Col>
          <Col span="20" class="item-content">{{desc.briefing}}</Col>
        </Row>
        <!-- <Row class="item mt20">
          <Col span="4" class="label-name">农家乐简介</Col>
          <Col span="20" class="item-content">{{desc.summary}}</Col>
        </!--> 
        <Row class="item mt20">
          <Col span="4" class="label-name">农家乐介绍</Col>
...
import { agritainmentView, orderAddress, basicView } from "@service/place";
...
export default class detail extends Vue {
  ...
  getDesc(id: any) {
    ...
    agritainmentView({ id: id }).then((res: any) => {
      if (res.code === 0) {
        _this.desc = res.data.ctcAgritainment;
        let _data = res.data.ctcAgritainment;
        ...
        _this.recordsLog = res.data.auditRecordsVo.recordsLog;
         //设施标签
        if (res.data.ctcAgritainment.serviceFacility) {
          this.getTags(res.data.ctcAgritainment.serviceFacility, "eqtTagList");
        }
        //类型标签
        if (res.data.ctcAgritainment.type) {
          this.getTags(res.data.ctcAgritainment.type, "type");
        }
      } else {
