<template>
  <div>
    <baseTable :configure="tableConfig" @getList="getList">
      ...
      >
        <template slot-scope="{row,index}" slot="toiletMsg">
          <div class="clearfix toilet-msg">
            <div class="msg-panel">
              <p class="toilet-name">
                <router-link
                  :to="{path:'/toilet-detail',query:{id:row.id,type:'list'}}"
                  class="theme-blue toilet-tag"
        ...
        <template slot-scope="{row,index}" slot="action">
          ...
          >提交</a>
          <router-link
            :to="{path:'/toilet-edit',query:{id:row.id,type:'list'}}"
...
import {
  ...
  submitBatch,
  toiletWithdraw,
  getToiletAuditProcess,
  ctcToiletImportExcel
} from "@service/place";
...
export default class toiletTable extends Vue {
  // 权限列表
  get operates() {
  ...
  private outExcelApi: string =
    CONFIG.defaultUrl + "/res/site/ctcToilet/exportExcel?";
  outExcel(e: any) {
  ...
  private acceptExcel(e: any) {
    ...
    this.spinShow = true;
    ctcToiletImportExcel(formData).then((blob: any) => {
  ...
  getList(data: any = this.tableConfig.page) {
    ...
    getToiletList(this.params).then((res: any) => {
      if (res.code === 0) {
  ...
  submitCol(item: any, index: any) {
    let _this = this,
      checkedLen = _this.checkedIds ? _this.checkedIds.split(",").length : 0;
    if (index === -1 && !checkedLen) {
      _this.$Message.error("请选择你要提交的厕所");
      return;
    }
    let config: any = {
      title: "操作提示",
  ...
  mounted() {
    let _this = this;
    //搜索条件
    _this.bus.$on("requestToiletList", (res: any) => {
      this.tableConfig = {
        page: {
          currPage: 1,
          pageSize: 10,
