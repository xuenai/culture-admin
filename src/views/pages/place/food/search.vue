    ...
    <div class="wrap">
        <Form :label-width="50" inline>
            ...
            <FormItem class="item" label="名称">
                <Input  placeholder="请输入餐饮名称" v-model="data.name" @on-enter="handleSubmit"/>
            </FormItem>
            ...
            <!--<FormItem class="item" label="等级：">-->
                <!--<Select class="w150" v-model="data.type">-->
                    <!--<Option value="" selected>全部</Option>-->
                    <!--<Option value="五星级" selected>五星级</Option>-->
            ...
            <div class="submit">
                ...
                <router-link :to="{path:'/food-search', query:{id:id}}" v-if="operates.includes('save') " class="btn-primary ml10">添加</router-link>
                <!-- <router-link :to="{path:'/food-add', query:{id:id}}" v-if="operates.includes('save') && $store.state.app.userInfo.dataType !== 'SITE'" class="btn-primary ml10">添加</router-link> -->
            </div>
        </Form>
    </div>
</template>
...
<script lang="ts">
    ...
    export default class foodSearch extends Vue {
        ...
        handleSubmit(){
            this.bus.$emit('requestFoodList',this.data)
        }
        getDate(value:any){
