<template>
  <div>
    <baseTable :configure="tableConfig" @getList="getList">
      ...
      >
        <template slot-scope="{row,index}" slot="hotelMsg">
          <div class="clearfix hotel-msg">
            <div class="msg-panel">
              <p class="hotel-name">
                <span
                  class="red"
                ...
                <router-link
                  :to="{path:'/hotel-detail',query:{id:row.id,type:'list'}}"
        ...
        <template slot-scope="{row,index}" slot="action">
          <!-- <router-link :to="{path:'/guestroom-list',query:{hotelId:row.id}}" v-if="operates.includes('guestRoom') && row.status==1" class="theme-blue action-btn">客房</router-link> -->
          <dropCode
            v-if="row.status === 1"
            :id="'qrcode' + row.id"
            :key="'qrcode' + row.id"
            :value="$store.state.app.userInfo.h5Domain+'/#/hotel-detail/' +row.id"
          >
          ...
          >提交</a>
          <router-link
            :to="{path:'/hotel-edit',query:{id:row.id,type:'list'}}"
...
export default class hotelTable extends Vue {
  // 权限列表
  get operates() {
  ...
  private outExcelApi: string =
    CONFIG.defaultUrl + "/res/site/ctcHotel/exportExcel?";
  outExcel(e: any) {
  ...
  private acceptExcel(e: any) {
    ...
    this.spinShow = true;
    hotelImportExcel(formData).then((blob: any) => {
  ...
  getList(data: any = this.tableConfig.page) {
    ...
    getHotelList(this.params).then((res: any) => {
      if (res.code === 0) {
  ...
  deleteRow(item: any, index: any) {
    ...
    if (index === -1 && !checkedLen) {
      _this.$Message.error("请选择你要删除的酒店");
      return;
    }
    let config: any = {
      title: "操作提示",
  ...
  submitCol(item: any, index: any) {
    let _this = this,
      checkedLen = _this.checkedIds ? _this.checkedIds.split(",").length : 0;
    if (index === -1 && !checkedLen) {
      _this.$Message.error("请选择你要提交的酒店");
      return;
    }
    let config: any = {
      title: "操作提示",
  ...
  mounted() {
    let _this = this;
    //搜索条件
    _this.bus.$on("requestHotelList", (res: any) => {
      this.tableConfig = {
        page: {
          currPage: 1,
          pageSize: 10,
