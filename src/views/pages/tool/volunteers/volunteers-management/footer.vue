...
<template>
  <div class="btn-box">
    <Button type="primary" v-if="!order && operates.includes('recommend')" @click="setRecommendStatus">推荐</Button>
    <Button type="primary" v-if="order && operates.includes('recommend')" :disabled="true">推荐</Button>
    <Button type="primary" :disabled="order" v-if="operates.includes('cancelRecommend')" @click="unRecommendStatus">取消推荐</Button>
    <Button type="primary" :disabled="order" v-if="operates.includes('top')" @click="setTopStatus(1,'置顶')">置顶</Button>
    <Button type="primary" :disabled="order" v-if="operates.includes('cancelTop')" @click="setTopStatus(0,'取消')">取消置顶</Button>
...
import { setVolunteerRecommend, setVolunteerTop, setVolunteerSort } from "@service/tool";
...
export default class FooterButtons extends Vue {
  ...
  private setRecommendStatus(name: string) {
    ...
    let config: any = {
      ...
      cancelText: "取消",
      onOk: () => {
        setVolunteerRecommend(params).then((res: any) => {
          if (res.code === 0) {
  ...
  private setTopStatus(top: Number,name: string) {
    ...
    let config: any = {
      ...
      cancelText: "取消",
      onOk: () => {
        setVolunteerTop(params).then((res: any) => {
          if (res.code === 0) {
