...
import { getSchoolCourse } from "@service/tool";
@Component({
  components: {
    auditHead,
    detailContent,
...
export default class auditResultHead extends Vue {
  ...
  private type: string = "CONTENT_TYPE_COURSE";

  created() {
    this.getSchoolCourse(this.$route.params.id);
    this.isAudit = String(this.$route.query.isAudit) === "true";
  }

  // 获取详情
  private getSchoolCourse(id: any) {
    getSchoolCourse({ id }).then((res: any) => {
      if (res.code === 0) {
        ...
        this.recordList = auditData.recordsLog;
        this.editBtnStatus = [3, 6, 8].includes(auditData.currentAuditStatus);
        this.auditParams = {
