<template>
  <div>
    <Form class="form" ref="params" :model="params" :rules="ruleValidate" :label-width="90">
      <FormItem label="讲师名字" prop="name">
        <Input placeholder="请输入讲师名字" v-model="params.name" :maxlength="30" class="w300" @on-enter="getList()" />
        <Button @click="getList()" type="primary" class="ml10">搜索</Button>
        <Button v-if="operates.includes('save')" @click="$router.push({path: '/lecturer-add'})" type="primary" class="ml10">添加</Button>
      </FormItem>
    </Form>
    
    <BaseTable class="list-box" :configure="tableConfig" @getList="getList">
      <Table :columns="tableData.headList" :data="tableData.bodyList" size="default" :loading="loading">
        ...
        <template slot-scope="{ row }" slot="action">
          <router-link v-if="operates.includes('edit')" :to="{path:'/lecturer-add', query: {id: row.id}}" class="theme-blue">修改
          </router-link>
...
import { getSchoolLecturerList, deleteSchoolLecturer, updateSchoolLecturer } from "@service/tool"
...
export default class BrandAdd extends Vue {
  ...
  private getList(page?: any) {
    (this.$refs.params as any).validate((valid: boolean) => {
      if (valid) {
        ...
        this.loading = true
        getSchoolLecturerList(this.params).then((res:any) => {
          if (res.code === 0) {
  ...
  private deleteIds(ids: any) {
    let config = <any>{
      ...
      cancelText: "取消",
      onOk: () => {
        deleteSchoolLecturer({ ids }).then((res: any) => {
          if (res.code === 0) {
  ...
  private changeStatus(row: any, title: string) {
    let config = <any>{
      ...
      okText: "确认",
      cancelText: "关闭",
      onOk: () => this.updateSchoolLecturer(row)
    };
    this.$Modal.confirm(config);
  }
  private updateSchoolLecturer(row:any) {
    updateSchoolLecturer({
      id: row.id,
