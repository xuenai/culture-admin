  ...
  <div class="analysis">
    ...
    <div class="analysis-content">
      <ul class="analysis-content-list">
        ...
        <li class="analysis-content-item">
          <p>点评总数 <Poptip trigger="hover" word-wrap :width="150" content="用户每点评1个页面，记录1个点评数。用户多次点评同一个页面，点评数累计多次。"><Icon class="tip" custom="iconfont c-icon--quetion-pane" size="12" /></Poptip></p>
          <p>
    ...
    <BaseTable :configure="configure" @getList="getList">
      <Table :columns="columns" :data="datas" :loading="loading" @on-sort-change="onSortChange">
        <template slot="name" slot-scope="{row}">
          <div><span class="text-red" v-if="row.top">[置顶]</span><span class="text-red" v-if="row.recommend">[推荐]</span><router-link class="mr-8" :to="{path: '/story-list', query: {tag: row.name}}" target="_blank">{{row.name}}</router-link></div>
        </template>
      </Table>
    </BaseTable>
  </div>
...
export default class ContentAnalysis extends Vue {
   ...
   */
  exportFile () {
    exportTagAnalyzeList(this.params).then(res => {
      if (res.code === 0) {
        this.$Message.success('导出标签分析数据成功！')
      }
    })
  }
  /**
   ...
   */
  getCount () {
    getTagAnalyzeCount(this.params).then(res => {
