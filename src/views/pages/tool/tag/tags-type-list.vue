...
<template>
  <div class="tags-page">
    ...
    <Modal v-model="addTagModle" :title="modalTitle" :width="400" footer-hide>
      <Form ref="modalData" :model="modalData">
        <FormItem label="标签名称：" :label-width="90">
          <Input v-model="modalData.labelName" :maxlength="10" placeholder="请输入标签名称"/>
        </FormItem>
...
export default class CloudTags extends Vue {
  ...
  private resetModalData () {
    ...
    this.modalData.id = ''
    // (this.$refs.modalData as Vue).resetFields();
  }
  // 重新设置请求页数为1
  ...
  //   labelName: [{ required: true, message: "该项不能为空"}],
  //   labelType: [
  ...
  // 进行标签的新增或者修改操作
  private saveTag () {
    if(!this.modalData.labelName || !this.modalData.labelType){
