<template>
  <div class="container">
    ...
    <Tabs type="card" class="tabs" v-model="currTab">
      <TabPane label="景区和非遗" name="getScenicPt">
        <BaseTable class="list-box" :configure="tableConfig_scenicpt" @getList="getScenicPt">
          <Table border :columns="tableData_scenicpt.headList" :data="tableData_scenicpt.bodyList"
            ...
            <template slot-scope="{ row }" slot="action">
              ...
              <a @click="deleteGuidedTourResource(row)" href="javascript:;" class="control" style="margin: 0 10px">删除</a>
            </template>
          </Table>
        </BaseTable>
        <Button type="primary" long style="margin-top: 20px" @click="resourcePopShow = true">点击添加资源</Button>
      </TabPane>
            ...
            <template v-if="line.points && line.points.length">
              <ul class="point-list">
                <li class="line" v-for="(pt, i) in line.points" :key="pt.key"
                  ...
                  <div>
                    <InputNumber class="input" :precision="0" :min="1" :max="5000" v-model="pt.playTime"></InputNumber> 分钟</div>
                  <div>
      ...
      <Button size="small" slot="extra" @click="downLoad">下载景区二维码</Button>
    </Tabs>
    ...
    <!-- 资源添加弹窗 -->
    <ScenicRemains :tourId="id" v-model="resourcePopShow" @select="list => getScenicPt()"></ScenicRemains>
  </div>
</template>
...
import {
  ...
  deleteGuidedTourResource
} from "@service/tool";
import { exportExcel } from "@util/comm"
import CONFIG from "@/util/config";
import BaseTable from "@components/baseTable/baseTable.vue";
import ScenicRemains from "@components/pop/scenic+remains.vue";
// @ts-ignore
...
export default class Setting extends Vue {
  // 景点非遗添加资源弹窗
  private resourcePopShow: boolean = false;
  // 当前选项卡
  ...
  private addLineMarker (clearAllOverlay = false) {
    ...
    if (this.currLineId) {
      ...
      let marker = points.map((pt:any) => {
        ...
        if (['CONTENT_TYPE_SCENIC_SPOTS', 'CONTENT_TYPE_SCENERY'].includes(pt.resourceType)) {
          icon = icon_scenicPt
  ...
  private getScenicPt(page?: any) {
    ...
    getGuidedTourResource(params).then((res: any) => {
      if (res.code === 0) {
      ...
      (res: any) => {
        if (res.code === 0) {
          res.datas.forEach((pt: any) => {
            ...
            pt.key = pt.resourceType + "_" + Math.random();
            pt.red = !this.validateLonLat(pt.position)
  ...
  // 删除景区和非遗
  private deleteGuidedTourResource(row: any) {
    this.$Modal.confirm({
      title: '系统提示',
