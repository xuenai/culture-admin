<template>
    <div class="container pd15">
        <Form class="wrap" :label-width="80" inline>
            <FormItem class="item" label="编号">
                <Input placeholder="社团编号" v-model="data.code" @on-enter="getList"/>
            </FormItem>
            <FormItem class="item" label="名称">
                <Input placeholder="社团名称" v-model="data.name" @on-enter="getList"/>
            </FormItem>
            <FormItem class="item" label="创建者">
                <Input placeholder="创建者名称" v-model="data.createUser" @on-enter="getList"/>
            </FormItem>
            <FormItem class="item" label="类型">
                <Select class="w150" v-model="data.type" @on-change="getList">
                    <Option value="">全部</Option>
                    <Option :value="item.value" v-for="(item,index) in types" :key="index" v-if="item.pid">
                        {{item.name}}
                    </Option>
                </Select>
            </FormItem>
            ...
            <FormItem class="item" label="成立时间">
                <Date-picker type="daterange" placement="bottom-end" placeholder="成立时间" @on-change="getAddTime"
                             :value="addTime"></Date-picker>
            </FormItem>
            <FormItem class="item" label="提交时间">
                <Date-picker type="daterange" placement="bottom-start" placeholder="提交时间" @on-change="getCreateTime"
                             :value="createTime"></Date-picker>
            </FormItem>
            <div class=" submit">
                <Button type="primary" @click="getList">搜索</Button>
                <router-link :to="{path:'/community-add'}" class="btn-primary ml10">添加</router-link>
            </div>
        </Form>
        <baseTable :configure="tableConfig" @getList="getList">
            <Table class="table mt15" border :columns="tableData.headList" :data="tableData.bodyList"
                ...
                <template slot-scope="{row,index}" slot="communityMsg">
                    <div class="clearfix venues-msg">
                        <div class="msg-panel">
                            <p class="venues-name">
                                <span class="red"
                                      v-if="row.recommendHomePage === 1 ||row.recommendChannelHomePage === 1 ">[推荐]</span>
                                <span class="red" v-if="row.top === 1">[置顶]</span>
                                <router-link :to="{path:'/community-detail',query:{id:row.id,type:'list'}}"
                                             class="theme-blue venues-tag">{{row.name}}（{{row.code}}）
                                </router-link>
                            </p>
                            <p class="address">
                ...
                <template slot-scope="{row,index}" slot="action">
                    <router-link tag="a" :to="{path:'/community-member-list',query:{communityId:row.id}}" class="theme-blue action-btn"
                                 v-if="row.status===1&&operates.includes('ass_person')">成员</router-link>
                    <a class="theme-blue action-btn" v-if="row.status===3&&operates.includes('submit')" @click="submitCol(row,index)">提交</a>
                    <dropCode
                        v-if="row.status === 1"
                        :id="'qrcode' + row.id"
                        :key="'qrcode' + row.id"
                        :value="$store.state.app.userInfo.h5Domain+'/#/group-desc/' +row.id"
                    >
                    <span class="theme-blue action-btn">预览</span>
                    </dropCode>
                    <!--<span class="theme-blue action-btn" >预览</span>-->
                    <router-link class="theme-blue action-btn" tag="a" :to="{path:'/community-add',query:{id:row.id,type:'list'}}"
                                 v-if="operates.includes('edit')">修改
                    </router-link>
        ...
        <footerButtons
                ...
                @getList="getList"
        >
            <Button type="primary" :disabled="order" @click="deleteRow(checkedIds,-1)" v-if="operates.includes('batchDelete')">批量删除
            </Button>
            <Button type="primary" :disabled="order" @click="submitCol(checkedIds,-1)" v-if="operates.includes('batchSave')">批量提交
            </Button>
            <Button type="default" class="ml10 import-panel"  :disabled="order" v-if="operates.includes('import')">
                <input class="upload-excel" type="file" @change="acceptExcel($event)" >导入
            </Button>
            <Button type="default"  class="ml10" @click="outExcel($event)" :disabled="order" v-if="operates.includes('export')">导出</Button>
        </footerButtons>
    </div>
</template>
...
<script lang="ts">
    ...
    import {dictType,updateStatus} from "@service/common";
    import {submitBatch,deleteCol} from "@service/place";
    import {associationList,updateDataStatus,associationImportExcel,associationSetSort} from "@service/tool";
    ...
    export default class commounitylist extends Vue {
        ...
        getList(data: any = this.tableConfig.page) {
            ...
            _this.loading = true;
            associationList(_this.data).then((res: any) => {
                if (res.code === 0) {
        ...
        //推荐，置顶，提交，删除，更新状态 公共接口请求方法
        private updateStatus(params:any,tips:string){
            let _this=this;
            updateDataStatus(params).then((res: any) => {
                if (res.code === 0) {
       ...
       //排序
        ...
        /*S 导出*/
        private outExcelApi: string = CONFIG.defaultUrl + '/res/site/ctcAssociation/exportExcel?';
        outExcel(e:any){
        ...
        private acceptExcel (e:any) {
            ...
            this.spinShow = true;
            associationImportExcel(formData).then((blob:any) => {
