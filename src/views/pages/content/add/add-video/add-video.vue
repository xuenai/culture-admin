<template>
  <div class="add-content-box">
    <Form class="content list" ref="params" :model="params" :rules="ruleValidate"
      ...
      <FormItem class="formItem img-check" label="视频">
        <UploadVideo3 @getVideoList="getVideoList" :defaultVideo="defaultVideo" :limit="1">
        </UploadVideo3>
        <div class="ivu-form-item-error-tip" :style="{display: imgMustTip}">请选择或上传视频</div>
      </FormItem>
      ...
      <template v-for="item in extendFieldList">
        ...
        <FormItem v-if="item.fieldType === '内容'" class="formItem" :prop="item.fieldCode"
          :label="item.fieldName" :key="item.id">
...
export default class AddVideo extends Vue {
  ...
  private saveMultiVideo() {
    return new Promise((resolve, reject) => {
      if (this.videoList && this.videoList.length) {
        ...
        if (list.length) {
          saveMultiVideo({ jsonDto: JSON.stringify(list) })
            .then(res => {
              if (res.code === 0 && res.datas) {
                //                            this.params.video = res.datas.map((data:any)=>data.ID).join(',')
                resolve();
              } else {
                reject(res.message);
              }
  ...
  private async submitForm(status?: number) {
    ...
    /*this.saveMultiVideo().then(()=>{
                ...
                if(this.$route.query.audit){
                    this.params.auditManageId = this.$route.query.auditId||'';
                    saveAuditUpdateData(this.params).then(data => {
                        if (data.code === 0) {
                    ...
                    saveAddContent(this.params).then(data => {
                        if (data.code === 0) {
                            this.$Message.success("保存成功！");
                             this.back()
                        }
    ...
    this.saveMultiVideo(); // 看看要不要保存资源库
  ...
  private renderContent() {
    ...
    if (video) {
      /*let promiseList = String(video).split(',').map((id:any) => this.getMediaDetail(Number(id)))
                Promise.all(promiseList).then((list: Array<any>) => {
                    this.defaultVideo = list.map((data:any) => {
                        data.isOld = true;
                        return data
                    })
                    this.getVideoList(this.defaultVideo); // 解决编辑后直接保存 校验不通过的问题
