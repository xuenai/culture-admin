<template>
  <div class="add-content-box">
    <Form class="content list" ref="params" :model="params" :rules="ruleValidate"
      ...
      <FormItem class="formItem img-check" label="音频">
        <UploadVideo3 @getVideoList="getVideoList" :defaultVideo="defaultVoice" sourceType="voice"
          :limit="1"></UploadVideo3>
        <div class="ivu-form-item-error-tip" :style="{display: imgMustTip}">请选择或上传音频</div>
      </FormItem>
...
import UploadVideo3 from "@components/upload/upload-video3.vue";
...
export default class AddAudio extends Vue {
      ...
      if (this.videoList && this.videoList.length) {
        let list = this.videoList
          ...
          .map((video: videoType) => ({
            ...
            placeType: video.placeObj
              ? video.placeObj.resourceType
              : video.placeType
              ? video.placeType
              : "",
            placeName: video.placeObj
              ? video.placeObj.name
              : video.placeName
              ? video.placeName
              : "",
            ...
            tags: video.tags,
            id: video.id, // 由原来的新增变成了修改
            orgId: this.params.createCompany
          })); // 现在需求是：本地添加的资源才保存资源库（没得id就是本地加的）
        if (list.length) {
          saveMultiVoice({ jsonDto: JSON.stringify(list) })
            .then((res: any) => {
              if (res.datas && res.datas.length) {
                /*let datas:Array<{ID: number}> = res.datas
                                 this.videoList && this.videoList.forEach((item: videoType, i: number) => {
                                 item.id = datas[i].ID
                                 })
                                 this.params.audio = datas.map((data:any) => (data.ID || '')).filter((id:any)=>!!id).join(',')*/
                resolve();
              } else {
  ...
  // 选择media
  private getVideoList(list: Array<any>) {
    this.videoList = list.map((item: any) => {
      if (item.isLocalAdd === true) item.isOld = true; // 如果是本地添加的资源，那么我们直接使用这个资源保存之后的id，没必要再建一条新数据
      return item;
    });
  ...
  private async submitForm(status?: number) {
    ...
    this.saveMultiVoice(); // 看看要不要保存资源库

    /*后台需要标签的名字，先把标签名字查出来S*/
    let allPromise: Array<Promise<any>> = [];
    (this.videoList as Array<any>).forEach((data: any) => {
      data.tagNames = [];
      if (data.tags) {
        allPromise.concat(
          data.tags.split(",").map(
    ...
    this.params.audio = JSON.stringify(
      (this.videoList as Array<any>).map((data: any) => ({
        imgUrl: data.imgUrl,
        name: data.name,
        url: data.url,
        mediaId: data.id,
  ...
  private renderContent() {
    ...
    if (audio) {
      // 回血媒体
      // 接口改了，这段不适用了
      /*let promiseList = String(audio).split(',').map((id:any) => this.getMediaDetail(Number(id)))
                Promise.all(promiseList).then((list: Array<any>) => {
                    this.videoList = this.defaultVoice = list.map((data:any) => {
                        data.isOld = true;
                        return data
                    })
      ...
      this.getVideoList(this.defaultVoice);
    }
    if (extendInfo) {
      // 回显扩展字段值
  ...
  private getContentColumnList(callback?: () => void) {
    getContentColumnList().then((data: any) => {
