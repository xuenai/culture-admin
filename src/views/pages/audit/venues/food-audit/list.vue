        ...
        <baseTable :configure="tableConfig" @getList="getList">
            <Table border :columns="tableData.headList" :data="tableData.bodyList" class="table"
                ...
                <template slot-scope="{row}" slot="name">
                    <router-link :to="{path:'/audit-food-edit',query:{id:row.resourceId,resourceName:row.name}}" class="theme-blue action-btn ml5">{{row.name}}({{row.code}})</router-link>
                </template>
                ...
                <template slot-scope="{row,index}" slot="action">
                    <router-link :to="{path:'/audit-food-edit',query:{id:row.resourceId,resourceName:row.name}}" v-if="operates.includes('audit')&&row.status==4" class="theme-blue action-btn">审核</router-link>
                    <a class="theme-blue action-btn ml5" v-if="operates.includes('process') " @click="getFoodAuditProcess(row.resourceId)">流程</a>
                    <router-link :to="{path:'/audit-food-modify',query:{id:row.resourceId,type:'audit'}}" v-if="operates.includes('edit')&&row.status!=7&&row.status!=9 " class="theme-blue action-btn ml5">修改</router-link>
...
<script lang="ts">
    ...
    import {getDiningAuditProcess} from '@service/place';
    ...
    export default class vunuesAudit extends Vue {
        ...
        private getFoodAuditProcess(id: any) {
            this.modal = true;
