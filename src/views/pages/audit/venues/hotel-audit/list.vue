<template>
    <div class="venues-audit">
        ...
        <baseTable :configure="tableConfig" @getList="getList">
            <Table :columns="tableData.headList" :data="tableData.bodyList" class="table"
                    :loading="loading" :row-class-name="rowClassName">
                <template slot-scope="{row,index}" slot="name">
                    <!--酒店详情-->
                    <router-link :to="{path:'/audit-hotel-edit',query:{id:row.resourceId,resourceName:row.name}}"
                                    v-if="row.resourceType=='CONTENT_TYPE_HOTEL'"
                                    class="theme-blue action-btn ml5">{{row.name}}({{row.code}})
                    </router-link>
                    <!--客房详情-->
                    <router-link :to="{path:'/audit-guestroom-edit',query:{id:row.resourceId,hotelId:row.hotelId,resourceName:row.name}}"
                                    v-else-if="row.resourceType=='CONTENT_TYPE_HOTEL_ROOM'"
                                    class="theme-blue action-btn ml5">{{row.name}}({{row.code}})
                    </router-link>
                </template>
                <template slot-scope="{row,index}" slot="integrity">
                ...
                <template slot-scope="{row,index}" slot="action">
                    <div v-if="row.status!=6">
                        <!--酒店审核-->
                        <router-link
                                :to="{path:'/audit-hotel-edit',query:{id:row.resourceId,resourceName:row.name}}"
                                v-if="row.resourceType=='CONTENT_TYPE_HOTEL'&&operates.includes('audit')&& row.status==4"
                                class="theme-blue action-btn">审核
                        </router-link>
                        <!--客房审核-->
                        <router-link
                                :to="{path:'/audit-guestroom-edit',query:{id:row.resourceId,hotelId:row.hotelId,resourceName:row.name}}"
                                v-if="row.resourceType=='CONTENT_TYPE_HOTEL_ROOM'&&operates.includes('audit')&& row.status==4"
                                class="theme-blue action-btn">审核
                        </router-link>
                        <a class="theme-blue action-btn ml5"
                            @click="getHotelAuditProcess(row.resourceId,row.resourceType)"
                            v-if="operates.includes('process') ">流程</a>
                        <!--酒店修改-->
                        <router-link :to="{path:'/audit-hotel-modify',query:{id:row.resourceId,type:'audit'}}"
                                        v-if="row.resourceType=='CONTENT_TYPE_HOTEL'&& operates.includes('edit')&& row.status!=7&&row.status!=9 "
                                        class="theme-blue action-btn ml5">修改
                        </router-link>
                        <!--客房修改-->
                        <router-link :to="{path:'/audit-guestroom-modify',query:{id:row.resourceId,hotelId:row.hotelId,type:'audit'}}"
                                        v-else-if="row.resourceType=='CONTENT_TYPE_HOTEL_ROOM'&& operates.includes('edit')&& row.status!=7&&row.status!=9"
                                        class="theme-blue action-btn ml5">修改
                        </router-link>
...
<script lang="ts">
    ...
    import {getHotelAuditProcess, getRoomAuditProcess} from '@service/place';
    ...
    export default class vunuesAudit extends Vue {
        ...
        deleteRow(item: any, resourceType: any) {
            let _this: any = this;
            let name: string = resourceType == 'CONTENT_TYPE_HOTEL' ? '酒店' : '客房';
            let config: any = {
                title: "操作提示",
        ...
        private getHotelAuditProcess(id: any, resourceType: string) {
            this.modal = true;
            if (resourceType == 'CONTENT_TYPE_HOTEL') {//酒店流程
                getHotelAuditProcess({id: id}).then((res: any) => {
        ...
        rowClassName(row: any) {
            if (row.resourceType == "CONTENT_TYPE_HOTEL") {
                return 'parent_row';
            } else {
                return 'child_row';
            }
