<template>
    <div class="perform-wrap">
        <!--客房详情-->
        <roomDetail :audit="audit"></roomDetail>
        <!--审核通过/不通过-->
        <div class="form-wrap pd20">
            <Form :label-width="100">
                ...
                <FormItem class="item mt20" label="审核备注：" v-if="data.isPass" :required="true" :error="error">
                    <Input type="textarea" :maxlength="150" :rows="4" class="remark" v-model="params.auditResult" placeholder="请写下审核备注，不通过时审核备注必须填写。"/>
                    <span>200字以内</span>
                </FormItem>
                ...
                <!-- <FormItem class="item submit mt20" v-if="!data.isPass">
                    <Button type="primary" class="btn w100" @click="saveAudit">审核</Button>
                    <Button type="info" class="btn ml20 w100" @click="goEdit">修改</Button>
                    <Button type="default" class="btn-cancel btn ml20 w100" @click="cancel">返回</Button>
                </FormItem>
                <FormItem class="item submit mt20" v-if="data.isPass">
                    <Button type="primary" class="btn w100" @click="saveAudit">保存</Button>
                    <Button type="default" class="btn-cancel btn ml20 w100" @click="cancel">返回</Button>
                </FormItem> -->
...
<script lang="ts">
    ...
    import hotelDetail from '@views/pages/place/hotel/hotel-index/detail.vue'
    import roomDetail from '@views/pages/place/hotel/guest-room/detail.vue'
    ...
    export default class auditPerform extends Vue {
        ...
        private params: any = {
            ...
            resourceId: '',//资源ID
            resourceName: '',//资源名称
            resourceType: 'CONTENT_TYPE_HOTEL_ROOM',//资源类型
            auditResult: '',//审核内容,
            auditType: 1
        };
        ...
        //修改
        goEdit(){
            this.$router.push({path: '/audit-guestroom-modify', query: {id:this.$route.query.id,hotelId: this.$route.query.hotelId, type: 'audit'}});
        }
        created(){
