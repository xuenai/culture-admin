        ...
        <baseTable :configure="tableConfig" @getList="getList">
            <Table border :columns="tableData.headList" :data="tableData.bodyList" class="table"
                ...
                <template slot-scope="{row}" slot="name">
                    <router-link :to="{path:'/audit-toilet-edit',query:{id:row.resourceId,resourceName:row.name}}" class="theme-blue"> {{row.name}}({{row.code}})</router-link>
                </template>
                ...
                <template slot-scope="{row,index}" slot="action">
                    <router-link :to="{path:'/audit-toilet-edit',query:{id:row.resourceId,resourceName:row.name}}" v-if="operates.includes('audit')&&row.status==4" class="theme-blue action-btn">审核</router-link>
                    <a class="theme-blue action-btn ml5" v-if="operates.includes('process') " @click="getToiletAuditProcess(row.resourceId)">流程</a>
                    <router-link :to="{path:'/audit-toilet-modify',query:{id:row.resourceId,type:'audit'}}" v-if="operates.includes('edit')&&row.status==4 " class="theme-blue action-btn ml5">修改</router-link>
...
<script lang="ts">
    ...
    import {getToiletAuditProcess} from '@service/place';
    ...
    export default class vunuesAudit extends Vue {
        ...
        getList(data:any=this.tableConfig.page){
            ...
            getAuditResourceList(this.params).then((res: any) => {
                if (res.code === 0) {
                    ...
                    _this.tableConfig.page = res.page;
                    _this.loading=false;
                    _this.getAuditCount();
                }
            });
            ...
            let config: any = {
                ...
                onOk: () => {
                    delVenuesAuditList({
                        ...
                        type:'CONTENT_TYPE_TOILET' //场馆资源类型
                    }).then((res:any)=>{
                        if(res.code===0){
        ...
        private getToiletAuditProcess(id: any) {
            this.modal = true;
            getToiletAuditProcess({ id: id }).then((res: any) => {
