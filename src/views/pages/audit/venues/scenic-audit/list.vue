            ...
            <baseTable :configure="tableConfig" @getList="getList">
                <Table :columns="tableData.headList" :data="tableData.bodyList" class="table"
                    ...
                    <template slot-scope="{row}" slot="name">
                        <!--景区详情-->
                        <router-link :to="{path:'/audit-scenic-edit',query:{id:row.resourceId,resourceName:row.name}}"
                                     v-if="row.resourceType=='CONTENT_TYPE_SCENERY'"
                                     class="theme-blue">
                            {{row.name}}({{row.code}})
                        </router-link>
                        <!--景点详情-->
                        <router-link
                                :to="{path:'/audit-attractions-modify',query:{id:row.resourceId,scenicId:row.scenicId,type:'audit'}}"
                                v-else-if="row.resourceType=='CONTENT_TYPE_SCENIC_SPOTS'"
                    ...
                    <template slot-scope="{row}" slot="action">
                        <div v-if="row.status!=6">
                            <!--景区审核-->
                            <router-link
                                    :to="{path:'/audit-scenic-edit',query:{id:row.resourceId,resourceName:row.name}}"
                                    v-if="row.resourceType=='CONTENT_TYPE_SCENERY'&&operates.includes('audit')&&row.status==4"
                                    class="theme-blue action-btn">审核
                            </router-link>
                            <!--景点审核-->
                            <router-link
                                    :to="{path:'/audit-attractions-edit',query:{id:row.resourceId,scenicId:row.scenicId,resourceName:row.name}}"
                                    v-if="row.resourceType=='CONTENT_TYPE_SCENIC_SPOTS'&&operates.includes('audit')&&row.status==4"
                                    class="theme-blue action-btn">审核
                            </router-link>
                            <!--景区详情-->
                            <a class="theme-blue action-btn ml5" v-if="operates.includes('process') "
                               @click="getScenicAuditProcess(row.resourceId,row.resourceType)">流程</a>
                            <!--景区修改-->
                            <router-link :to="{path:'/audit-scenic-modify',query:{id:row.resourceId,type:'audit'}}"
                                         v-if="row.resourceType=='CONTENT_TYPE_SCENERY'&&operates.includes('edit') && row.status!=7 && row.status!=9 "
                                         class="theme-blue action-btn ml5">修改
                            </router-link>
                            <!--景点修改-->
                            <router-link
                                    :to="{path:'/audit-attractions-modify',query:{id:row.resourceId,scenicId:row.scenicId,type:'audit'}}"
                                    v-else-if="row.resourceType=='CONTENT_TYPE_SCENIC_SPOTS'&&operates.includes('edit') && row.status!=7 && row.status!=9 "
                                    class="theme-blue action-btn ml5">修改
                            </router-link>
...
<script lang="ts">
    ...
    import {getScenicAuditProcess, getScenicSpotsAuditProcess} from '@service/place'
    ...
    export default class vunuesAudit extends Vue {
        ...
        deleteRow(item: any, resourceType: any) {
            let _this: any = this;
            let name: string = resourceType == 'CONTENT_TYPE_SCENERY' ? '景区' : '景点';
            let config: any = {
                title: "操作提示",
        ...
        private getScenicAuditProcess(id: any, resourceType: string) {
            this.modal = true;
            if (resourceType == 'CONTENT_TYPE_SCENERY') { //景区审核流程
                getScenicAuditProcess({id: id}).then((res: any) => {
                    this.processData = res.data;
                });
            } else {                                     //景点审核流程
                getScenicSpotsAuditProcess({id: id}).then((res: any) => {
                    this.processData = res.data;
                });
            }
        }
        ...
        rowClassName(row: any, index: number) {
            if (row.resourceType == "CONTENT_TYPE_SCENERY") {
                return 'parent_row';
            } else {
                return 'child_row';
            }
