        ...
        <baseTable :configure="tableConfig" @getList="getList">
            <Table border :columns="tableData.headList" :data="tableData.bodyList" class="table"
                ...
                <template slot-scope="{row}" slot="name"> 
                    <router-link :to="{path:'/audit-agritainment-edit',query:{id:row.resourceId,resourceName:row.name}}" class="theme-blue action-btn ml5">{{row.name}}({{row.code}})</router-link>
                    <p class="address">
                ...
                <template slot-scope="{row}" slot="integrity">
                    <Progress :percent="row.percent ? parseInt(row.percent) : 0" :stroke-width="5"></Progress>
                </template>
                ...
                <template slot-scope="{row,index}" slot="action">
                    <router-link :to="{path:'/audit-agritainment-edit',query:{id:row.resourceId,resourceName:row.name}}" v-if="operates.includes('audit')&&row.status==4" class="theme-blue action-btn">审核</router-link>
                    <a class="theme-blue action-btn ml5" v-if="operates.includes('process') " @click="getAgritainmentAuditProcess(row.resourceId)">流程</a>
                    <router-link :to="{path:'/audit-agritainment-modify',query:{id:row.resourceId,type:'audit'}}" v-if="operates.includes('edit')&&row.status!=7&&row.status!=9 " class="theme-blue action-btn ml5">修改</router-link>
                    <a class="theme-bluea action-btn ml5" v-if="operates.includes('delete')&&row.status!=4" @click="deleteRow(row,index)">删除</a>
                </template>
            </Table>
        </baseTable>
...
<script lang="ts">
    ...
    import { getAgritainmentStatus } from '@service/place';
    ...
    export default class vunuesAudit extends Vue {
        ...
        private params:object={
            type:'CONTENT_TYPE_AGRITAINMENT',
            code:'',//编号
            name:'',//名称
            status:'',//审核状态
            startTime:'',//开始时间
        ...
        deleteRow(item:any,index:any){
            ...
            if (index === -1 && !this.checkedBox.length) {
                _this.$Message.error("请选择你要删除的农家乐");
                return;
            }
            let config: any = {
                title: "操作提示",
        ...
        private getAgritainmentAuditProcess(id: any) {
            this.modal = true;
            getAgritainmentStatus({ id: id }).then((res: any) => {
