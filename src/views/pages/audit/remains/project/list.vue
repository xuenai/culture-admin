    ...
    <div class="venues-audit">
        ...
        <Form :label-width="80" inline class="wrap">
            ...
            <FormItem class="item" label="名称">
                <i-input placeholder="非遗项目名称" v-model="params.name"></i-input>
            </FormItem>
        ...
        <baseTable :configure="tableConfig" @getList="getList">
            <Table :columns="tableData.headList" :data="tableData.bodyList" class="table" :loading="loading" :row-class-name="rowClassName">
                <template slot-scope="{row}" slot="name">
                    <!--非遗项目详情-->
                    <router-link :to="{path:'/audit-project-detail',query:{id:row.resourceId,resourceName:row.name}}"
                                    v-if="row.resourceType=='CONTENT_TYPE_HERITAGE_ITEM'"
                                    class="theme-blue"> {{row.name}}({{row.code}})
                    </router-link>
                    <!--非遗传承人详情-->
                    <router-link :to="{path:'/audit-inheritor-detail',query:{id:row.resourceId,pid:row.pid,resourceName:row.name}}" v-else
                                    class="theme-blue">{{row.name}}
                    </router-link>
                </template>
                ...
                <template slot-scope="{row}" slot="action">
                    <div v-if="row.status!=6">
                        <!--非遗项目审核-->
                        <router-link
                                :to="{path:'/audit-project-edit',query:{id:row.resourceId,resourceName:row.name}}"
                                v-if="row.resourceType=='CONTENT_TYPE_HERITAGE_ITEM'&& row.status==4 "
                                class="theme-blue action-btn">审核
                        </router-link>
                        <!--传承人审核-->
                        <router-link
                                :to="{path:'/audit-people-edit',query:{id:row.resourceId,pid:row.pid,resourceName:row.name}}"
                                v-if="row.resourceType=='CONTENT_TYPE_HERITAGE_PEOPLE'&& row.status==4 "
                                class="theme-blue action-btn">审核
                        </router-link>
                        ...
                        <!--非遗项目修改-->
                        <router-link :to="{path:'/audit-project-modify',query:{id:row.resourceId,type:'audit'}}"
                                        v-if="row.resourceType=='CONTENT_TYPE_HERITAGE_ITEM' && row.status!=7 && row.status!=9 &&row.status!=6  "
                                        class="theme-blue action-btn ml5">修改
                        </router-link>
                        <!--传承人修改-->
                        <router-link :to="{path:'/audit-inheritor-modify',query:{id:row.resourceId,pid:row.pid,type:'audit'}}"
                                        v-else-if="row.resourceType=='CONTENT_TYPE_HERITAGE_PEOPLE'&& row.status!=7 && row.status!=9  "
                                        class="theme-blue action-btn ml5">修改
                        </router-link>
...
<script lang="ts">
    ...
    export default class vunuesAudit extends Vue {
        ...
        deleteRow(item: any, resourceType: any) {
            let _this: any = this;
            let name: string = resourceType == 'CONTENT_TYPE_HERITAGE_ITEM' ? '项目' : '传承人';
            let config: any = {
                title: "操作提示",
        ...
        private getVenueAuditProcess(id: any, resourceType: string) {
            this.modal = true;
            if (resourceType == 'CONTENT_TYPE_HERITAGE_ITEM') {//项目流程
        ...
        rowClassName(row: any, index: number) {
            if (row.resourceType == "CONTENT_TYPE_HERITAGE_ITEM") {
                return 'parent_row';
            } else {
                return 'child_row';
            }
