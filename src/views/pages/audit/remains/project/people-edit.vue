<template>
    <div class="perform-wrap">
        <!--非遗传承人详情-->
        <peopleDetail :audit="audit"></peopleDetail>
        <!--审核通过/不通过-->
        <div class="form-wrap pd20">
            <Form ref="formData" :label-width="100"  :model="params">
                ...
                <FormItem class="item submit mt20" v-if="!data.isPass">
                    <Button type="primary" class="btn" @click="saveAudit(1)">审核</Button>
                    <Button v-if="isSITE && userInfo.superSubmitButton" @click="saveAudit(2)" type="info">审核并提交上级审核</Button>
                    <Button type="info" class="btn ml20" @click="goEdit">修改</Button>
                </FormItem>
                <FormItem class="item submit mt20" v-if="data.isPass">
                    <Button type="primary" class="btn" @click="saveAudit(1)">保存</Button>
...
<script lang="ts">
    ...
    import peopleDetail from '@views/pages/activity/remains/people/detail.vue'
    ...
    export default class auditPerform extends Vue {
        ...
        private params: any = {
            ...
            resourceId: '',//资源ID
            resourceName: '',//资源名称
            resourceType: 'CONTENT_TYPE_HERITAGE_PEOPLE',//资源类型
            auditResult: '',//审核内容
            auditType: 1,//审核方式
        };
        //保存审核
        ...
        saveAudit(auditType = 1) {
           ...
           if((_this.data as any).isPass && !(_this.params as any).auditResult){
               ...
               _this.params.auditType = auditType
        ...
        //修改
        goEdit(){
            this.$router.push({path: '/audit-inheritor-modify', query: {id:this.$route.query.id,pid: this.$route.query.pid, type: 'audit'}});
        }
        created(){
