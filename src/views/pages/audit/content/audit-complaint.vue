<!--
 * @Author: 任智勇
 * @since: 2019-06-18 19:37:25
 * @LastAuthor: 任智勇
 * @lastTime: 2019-08-08 14:02:18
 -->
<template>
<!-- 审核-内容审核-投诉  -->
  <div class="review-audit">
    <div class="content">
      <Tabs active-key="key1" class="tab-warp" :animated="false" @on-click="setState">
        ...
        <Tab-pane :label="number04 > 0 ?  label04 : '待处理'"></Tab-pane>
        <Tab-pane label="已受理" class="tab-pane"></Tab-pane>
      </Tabs>
      <Form :label-width="100" inline class="form">
        ...
        <FormItem class="item" label="处理状态">
          <Select class="w200" v-model="params.status">
          ...
          >
            <div class="item-box">
              <div class="comment-content items">
                ...
                <div class="clearfix">
                  ...
                  <div class="img-list fl" v-if="item.evidenceImages && item.evidenceImages.length>=1">
                    <p class="img" v-for="(items,picInd) in item.evidenceImages" :key="items" >
                      <img :src="items" alt class="daq-img" @click.stop="viewImage(item.evidenceImages,picInd)">
                    </p>
                  </div>
                  <!-- 视频证据 -->
                  <div v-if="item.evidenceVideo && item.evidenceVideo.length>=1">
                    <div class="daq-no-photo view-images-box fl" v-for='(col,videoInd) in item.evidenceVideo' :key="videoInd" @click="openVideo(item, videoInd)">
              ...
              <p class="change items">
                <router-link
                :to="{path: '/audit-complaint-detail', query: {id: item.id}}"
                class="theme-blue action-btn"
                v-if="operates.includes('deal')"
                >处理</router-link>
              </p>
            </div>
...
export default class Complaint extends Vue {
  ...
  private status: any = DEAL_STATUS; // 处理状态
  ...
  private getListSet(status:number) {
    getComplaintsList({
      status: status
    }).then((data: any) => {
      if(status===4) {
  ...
  private openVideo(item: any, i: number) {
    this.showVideo.url = item.evidenceVideo[i];
    this.showVideo.image = item.evidenceVideoCover[i];
