      ...
      <baseTable :configure="tableConfig" @getList="getList">
        ...
        >
          <template slot-scope="{ row }" slot="title">
            ...
            <p class="reply line1" v-if="row.auditRemark && row.status === 79" :title="`审核备注${row.auditRemark}`">
              <span>审核备注：</span>{{ row.auditRemark }}
            </p>
          </template>
            <template slot-scope="{ row,index }" slot="point">
                <span v-if="creditData.length">{{creditData[index].creditScore}}</span>
                <span v-if="!creditData.length">-</span>
            </template>
          ...
          <template slot-scope="{ row }" slot="action">
            <div>
              <router-link
                ...
                >{{row.status === 4 ? '审核' : row.status !== 4 ? '重审' : '-'}}</router-link
              >
            </div>
          </template>
        </Table>
...
import { getVolunteerAuditNum } from "@service/audit";
import {getVolunteerManList} from '@service/tool'
...
export default class volAuditList extends Vue {
  ...
  // 获取通过的数量
  private getListSet() {
    getVolunteerAuditNum().then((data: any) => {
  ...
  private getList(page: any = this.tableConfig.page) {
    ...
    getVolunteerManList(this.params).then(data => {
      ...
      this.getCreditUser(phones.join(','));
