  ...
  <div class="right-panel">
    <Form :label-width="80" inline class="form mt20" :model="params" :rules="ruleValidate">
      <FormItem class="item" label="名称">
        <i-input placeholder="基地名称" class="w300" v-model="params.name"
      ...
      <FormItem class="item" label="批次">
        <Select class="w300" v-model="params.batch" @on-change="() => search()">
          <Option value="-1">全部</Option>
          <Option v-for="b in batch" :value="b.id" :key="b.id">{{ b.name }}
          </Option>
        </Select>
      </FormItem>
      ...
      <FormItem class="item" label="提交时间">
        ...
        <Button type="primary" class="ml20"
          @click="$router.push({path:'/protect-add'})">添加</Button>
      </FormItem>
    </Form>
    ...
    <BaseTable :configure="tableConfig" @getList="search">
      <Table border :columns="tableData.headList" :data="tableData.bodyList" size="default"
        ...
        <template slot-scope="{ row }" slot="info">
          <div class="title">
            <router-link :to="{path:'/protect-detail/' + row.id}" class="action-btn hot line6"
              :title="row.name">
        ...
        <template slot-scope="{ row }" slot="action">
          <dropCode v-if="[1,2].includes(row.status)" class="action-btn" :id="'qrcode' + row.id"
            :key="'qrcode' + row.id"
            :value="$store.state.app.userInfo.h5Domain+'/#/protect-desc/' +row.id">
          ...
          <router-link v-if="![4,5,9].includes(row.status) && row.showProcess === 1" :to="{path:'/protect-add', query: {id: row.id,saveType: 'list'}}" class="theme-blue">修改
          </router-link>
...
export default class volAuditList extends Vue {
  ...
  private acceptExcel(e: any) {
    ...
    ctcHeritageProtectBaseImport(formData)
      .then((blob: any) => {
  ...
  beforeRouteLeave (to:any, from:any, next: Function) {
    if ( /(\/protect-detail\/\d+)|(\/protect-add)/.test(to.path) ) {
      to.meta.menu = '/protect-list'
    }
    next()
  }
  // 参数对象
  ...
  private outExcelApi: string =
    CONFIG.defaultUrl + "/res/site/ctcHeritageProtectBase/exportExcel?";
