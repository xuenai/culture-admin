  ...
  <div class="right-panel">
    <Form :label-width="80" inline class="form mt20" :model="params" :rules="ruleValidate">
      <FormItem class="item" label="名称">
        <i-input placeholder="体验基地名称" class="w300" v-model="params.name"
      ...
      <FormItem class="item" label="提交时间">
        ...
        <Button type="primary" class="ml20"
          @click="$router.push({path:'/experience-add'})">添加</Button>
      </FormItem>
    </Form>
    ...
    <BaseTable :configure="tableConfig" @getList="search">
      <Table border :columns="tableData.headList" :data="tableData.bodyList" size="default"
        ...
        <template slot-scope="{ row }" slot="info">
          <div class="title">
            <router-link :to="{path:'/experience-detail/' + row.id}" class="action-btn hot line6"
              :title="row.name">
        ...
        <template slot-scope="{ row }" slot="action">
          <dropCode v-if="[1,2].includes(row.status)" class="action-btn" :id="'qrcode' + row.id"
            :key="'qrcode' + row.id"
            :value="$store.state.app.userInfo.h5Domain+'/#/experience-desc/' +row.id">
          ...
          <router-link v-if="![4,5,9].includes(row.status) && row.showProcess === 1" :to="{path:'/experience-add', query: {id: row.id,saveType: 'list'}}" class="theme-blue">修改
          </router-link>
...
import {
  ctcHeritageExperienceBaseImport,
  getExperienceBaseList,
  experienceWithdraw
...
export default class volAuditList extends Vue {
  ...
  beforeRouteLeave (to:any, from:any, next: Function) {
    if ( /(\/experience-detail\/\d+)|(\/experience-add)/.test(to.path) ) {
      to.meta.menu = '/experience-list'
    }
    next()
  }
  ...
  private acceptExcel(e: any) {
    ...
    ctcHeritageExperienceBaseImport(formData)
      .then((blob: any) => {
  ...
  private outExcelApi: string =
    CONFIG.defaultUrl + "/res/site/ctcHeritageExperienceBase/exportExcel?";
  ...
  private type: string = "CONTENT_TYPE_HERITAGE_EXPERIENCE_BASE";
  // 提交
