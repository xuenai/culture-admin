<template>
    <div class="container pd15">
        ...
        <Form class="mt15" ref="params" :label-width="150" :model="params" :rules="ruleValidate" @submit.native.prevent>
            <FormItem label="非遗项目名称：" prop="name">
                <Input class="control" v-model="params.name" placeholder="请输入非遗项目名称" @keyup.enter.native="handlerSearch"/>
            </FormItem>
            <FormItem label="行政区域：">
...
<script lang="ts">
    ...
    export default class list extends Vue {
        ...
        private ruleValidate: any = {
            name: [
                {required: true, message: "请输入非遗项目名称", trigger: "blur"},
        ...
        handlerSearch(){
            ...
            (this.$refs.params as any).validate((valid: any) => {
                ...
                _this.$router.push({path:'/project-search-list',query:{name:_this.params.name,region:_this.params.region,cityName:_this.params.cityName}});
            });
        }
    }
</script>
