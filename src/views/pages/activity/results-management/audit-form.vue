<template>
  <Form :label-width="80" ref="params" :model="params"
    ...
    <template v-if="resCode === 0">
      <FormItem label="审核备注" class="mt20" prop="auditResult" :key="1">
        <Input :key="11" v-model="params.auditResult" type="textarea" :maxlength="200"
      ...
      <div class="btn-group mt20">
        ...
        <Button @click="goEdit" v-if="editBtnStatus">修改</Button>
    ...
    <template v-else-if="resCode === 1">
      ...
      <FormItem label="审核备注" class="mt20" :key="2" prop="auditResult">
        <Input :key="21" v-model="params.auditResult" type="textarea" :maxlength="200"
          style="width: 250px;" :autosize="{minRows: 5,maxRows: 10}"
          placeholder="请写下审核备注，不通过时审核备注必须填写。" />
        <span>200字以内</span>
      </FormItem>
      <FormItem label="快捷回复">
        <Button class="mr10" v-for="(quick, i) in quickList" :key="i"
          @click="params.auditResult = quick">{{ quick }}</Button>
      </FormItem>
      <div class="btn-group mt20">
...
import { getQuickReply } from "@service/common";
...
export default class auditResultHead extends Vue {
  ...
  @Prop() private auditParams!: any;
  @Prop() private editBtnStatus!: boolean;
  @Prop() private goEdit!: Function;
  // 通过0 不通过1
  ...
  private quickList: string[] = [];
  private getQuickReply(): void {
    getQuickReply().then((res: any) => {
      if (res.code === 0) {
        this.quickList = res.datas as string[];
      }
    });
  }
  ...
  private activeAudit(auditStatus: number, auditType = 1) {
    (this.$refs.params as any).validate((valid: any) => {
      ...
      saveAuditProcess(Object.assign({}, this.params, this.auditParams)).then((res: any) => {
        if (res.code === 0) {
