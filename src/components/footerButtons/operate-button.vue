<template>
  <div class="btn-box">
    <Button type="primary" :disabled="order" v-if="operates.includes('recommend')"
      @click="setOperateStatus(1, 'RECOMMEND')">推荐</Button>
    <Button type="primary" :disabled="order" v-if="operates.includes('cancelRecommend')"
      @click="setOperateStatus(0, 'RECOMMEND')">取消推荐
    </Button>
    <Button type="primary" :disabled="order" v-if="operates.includes('top')"
      @click="setOperateStatus(1, 'TOP')">置顶</Button>
    <Button type="primary" :disabled="order" v-if="operates.includes('cancelTop')"
      @click="setOperateStatus(0, 'TOP')">取消置顶
    </Button>

    <Button type="primary" v-if="!order && operates.includes('sort')" @click="orderTrue">排序</Button>
    <Button type="primary" v-if="order && operates.includes('sort')"
      @click="orderClick">保存排序</Button>
    <slot></slot>
  </div>
</template>
...
export default class FooterButtons extends Vue {
  ...
  @Prop({ default: [] }) private selectionList!: Array<object>; // 选择数据
  ...
  private orderClick() {
    ...
    this.AllList.forEach(
      (d: any) => (params[d.id as number] = d.sort as number)
    );
    setSort({
  ...
  private setOperateStatus(status: number, type: OperateType) {
    ...
    let config: any = {
      ...
      cancelText: "取消",
      onOk: () => {
        if (type === OperateType.RECOMMEND) {
          setRecommendStatus({
            resourceType: this.resourceType,
            recommendHomePage: status,
            recommendChannelHomePage: status,
            ids: this.selectionList.map((d: any) => d.id).toString()
          }).then((res: any) => {
            if (res.code === 0) {
              this.$Message.success("操作成功");
