  ...
  <div class="ivu-transfer-list">
    ...
    <div class="ivu-transfer-list-body">
      <div class="ivu-transfer-list-content">
        <div class="ivu-transfer-list-content-item"
          v-for="item in showItems"
          :key="item.name"
          v-if="item.powerListVo.length"
        >
          <p class="type">{{item.name}}</p>
          <div class="ivu-transfer-list-content-item"
            v-for="vo in item.powerListVo" :key="vo.resourceId"
            @click.prevent="select(vo)"
            :title="item.pname ? `${vo.name}(${vo.pname})` : `${vo.name}`"
          >
            <Checkbox :value="isCheck(vo)"></Checkbox>
            <span class="name">{{vo.name}}<span v-if="vo.pname">（{{vo.pname}}）</span></span>
            <span class="theme" @click.stop="listItemClick(vo)" v-if="showResourceType && childResourceIsShow(vo.resourceType)">({{getResourceTxtByresourceType(vo.resourceType)}})</span>
          </div>
        </div>
        <div class="ivu-transfer-list-content-not-found">{{ notFoundText }}</div>
      </div>
    </div>
  </div>
</template>
...
@Component
export default class CTransferList extends Vue {
  // 列表标题
  ...
  get count () {
    ...
    let len = 0;
    this.data.map((e:any) => {
      len += e.powerListVo.length;
    })
    return (validKeysCount > 0 ? `${validKeysCount}/` : '') + `${len}`;
  }
  // 所有选中
