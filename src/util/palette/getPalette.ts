...
function sortByHue(a:Array<any>) {
  let l = a.length
  let b:Array<any> = []
  let c:Array<any> = []
  let j = 0
...
function binning(sortedData:any, total:any) {
  const binArr:Array<any> = []
  const rec = function (data:any) {
    const length = data.length
...
function shouldComplete(data:any) {
  const length = data.length
  const start = data[0][0]
  const end = data[length - 1][0]
  let total = 0
...
function mergeBins(binArr:Array<any>) {
...
function getDistance([r1, g1, b1]:any, [r2, g2, b2]:any) {
